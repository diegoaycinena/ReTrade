...
set more off
capture log close
log using "$my_path\ReTrade\ReTrade2\Log\do-crReTrade2_Sim3w2.log", replace

/* ReTrade Project (Stage II - Simulations)*/
/* Re-Trading, Price Discovery and Efficiency under Alternative Exchange Institutions */
/* Reads SIMULATIONS (wave 2) parameter level data */ 
/* creates SimParams0w2.dta file and merges to create SimSummaryW2.dta and SimPeriodSummaryW2.dta */
/* by Diego Aycinena */
/* Created: 2018-12-31 */
...
local FileName "Parameters_Data"	// Parameter data file contains records for all individual and market actions (bids actions, asks actions, trades, etc.) in a session
	...
	foreach FileDate of global `sim_treat'_f_date_listW2 {
		...
		**** Game Setup parameters [gameSettings]  ****
			****************************************	
			...
			if `i'==1 {
				save $path_data\SimGameSetup0w2.dta, replace
			}
			else {
				append using $path_data\SimGameSetup0w2.dta
				compress
				save $path_data\SimGameSetup0w2.dta, replace
			}
			sum numberOfPeriods
		...
		**** Period Setup parameters [period]  ****
			...
			*replace v_name = "p"+strtoname(v_name)
			...
			if `i'==1 {
				save $path_data\SimPeriodTime0w2.dta, replace
			}
			else {
				append using $path_data\SimPeriodTime0w2.dta
				compress
...
use $path_data\SimPeriodTime0w2.dta, clear
merge m:1 sim_run session using $path_data\SimGameSetup0w2.dta  //merge to create SimParams0.dta
...
save $path_data\SimParams0w2.dta, replace

*
use $path_data\SimPeriodSummary0w2.dta, clear //prepare to merge with Period Summary data

sort sim_run session period
egen maxP=max(period), by(session)

merge 1:1 sim_run session period using $path_data\SimParams0w2.dta  //merge

compare maxP numberOfPeriods
*assert maxP==numberOfPeriods // if period==1  --> 27 contradictions in 334 simulations
*drop maxP

*assert _merge==3
rename _merge MergeParamsPeriodSummary
...
save $path_data\SimPeriodSummaryW2.dta, replace

*tab 
use $path_data\SimSummary0w2.dta, clear //prepare to merge with Summary data

sort sim_run session period
egen maxP=max(period), by(session)

merge m:1 sim_run session period using $path_data\SimParams0w2.dta  //merge

*assert _merge==3 //match 99.3%
*drop _merge
rename _merge MergeParamsSummary


compare maxP numberOfPeriods
*assert maxP==numberOfPeriods // if period==1 & subject==1 --> 61 contradictions in 332 simulations
*drop maxP

compress 
save $path_data\SimSummaryW2.dta, replace
